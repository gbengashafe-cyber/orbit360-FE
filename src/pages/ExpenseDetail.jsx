import { ExpenseAttachment, ExpenseCategory, ExpenseLog, ExpenseRequest, User } from '@/api/entities';
...
import { Check, FileText, History, Loader2, MessageSquare, Paperclip, Printer, Upload, Wallet, X } from 'lucide-react';
              ...
              <div className="space-y-2">
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                  ...
                  <Input
                    ...
                    className="max-w-xs"
                  />
                  <p className="text-sm text-gray-500 mt-2">Upload receipts, invoices, or other supporting documents</p>
                </div>
              </div>
            )}
            ...
            {!isNew && (
              <div className="pt-4 border-t bg-gray-50 rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <p>
                    <strong>Submitted by:</strong> {request.requester_name}
                  </p>
                  <p>
                    <strong>Department:</strong> {request.department}
                  </p>
                  <p>
                    <strong>Request ID:</strong> {request.expense_id}
                  </p>
                  <p>
                    <strong>Current Status:</strong> {request.status?.replace(/_/g, ' ').toUpperCase()}
                  </p>
                </div>
              </div>
            )}
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-3">
                {canApprove && (
                  <>
                    <Button onClick={handleApproval} disabled={isProcessing} className="bg-green-600 hover:bg-green-700">
                ...
                {canMarkAsPaid && (
                  <Button onClick={handleMarkAsPaid} disabled={isProcessing} className="bg-purple-600 hover:bg-purple-700">
                    <Check className="w-4 h-4 mr-2" />
                    Mark as Paid
                  </Button>
                )}
                  ...
                  <li key={index} className="flex items-start">
                    <div className="flex flex-col items-center mr-4">
                      ...
                      {index < workflowSteps.length - 1 && (
                        <div className={`w-0.5 h-6 mt-2 ${index < currentStatusIndex ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                      )}
                    </div>
                    <div className="flex-1">
                      <h4 className={`font-medium text-sm ${index <= currentStatusIndex ? 'text-gray-900' : 'text-gray-500'}`}>
                        {step.label}
                      </h4>
                      {request?.status === step.status && <p className="text-xs text-blue-600 font-medium mt-1">Current Stage</p>}
                    </div>
                  </li>
                ))}
              </ul>
              ...
              {logs.map((log) => (
                <div key={log.id} className="p-3 bg-gray-50 rounded-lg border-l-4 border-blue-200">
                  <div className="flex justify-between items-start mb-1">
                    <p className="font-medium text-sm text-gray-900">{log.user_name}</p>
                    <p className="text-xs text-gray-500">{new Date(log.created_date).toLocaleDateString()}</p>
                  </div>
                  <p className="text-sm text-gray-600 capitalize">{log.action} the request</p>
                  {log.comment && (
