  ...
  const loadBaseData = async () => {
    ...
    try {
      ...
      if (userIsHrAdmin) {
        ...
        allEmpsData = Array.isArray(employeesResponse?.data)
        ...
        try {
          ...
          if (userEmployeeRecord && !userEmployeeRecord.departmentName) {
            userEmployeeRecord.departmentName = userEmployeeRecord.department?.name || userEmployeeRecord.departmentName || 'N/A';
          }
        } catch (err) {
                ...
                <SelectContent>
                  {selfEmployeeRecord && (
                    <SelectItem value="self">
                      View My Portal ({selfEmployeeRecord.firstName} {selfEmployeeRecord.lastName})
                    </SelectItem>
                  )}
                  {allEmployees.map((emp) => (
                    <SelectItem key={emp.id} value={String(emp.id)}>
                      {emp.firstName} {emp.lastName} ({emp.staffId || 'N/A'})
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
