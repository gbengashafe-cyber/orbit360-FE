...
import { PieChart as PieChartIcon, Users, Calendar as CalendarIcon, Briefcase, TrendingUp, DollarSign, Filter, Activity, UserMinus, BarChart2 } from 'lucide-react';
...
const MaterialTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-white rounded-lg p-4 shadow-lg border-0" style={{ 
...
const MetricCard = ({ title, value, icon: Icon, color, trend }) => (
  <div className={`bg-white rounded-lg p-6 ${ELEVATION[4]} hover:${ELEVATION[8]} transition-all duration-200 cursor-pointer transform hover:scale-105`}>
...
export default function Dashboard() {
  ...
  const processData = useCallback(() => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const terminationsThisMonth = employees.filter(e =>
        e.termination_date && new Date(e.termination_date) >= monthStart && new Date(e.termination_date) <= monthEnd
      ).length;

      const activeAtMonthStart = employees.filter(e =>
        new Date(e.hire_date) < monthStart && (!e.termination_date || new Date(e.termination_date) >= monthStart)
      ).length;

      const activeAtMonthEnd = employees.filter(e =>
        new Date(e.hire_date) <= monthEnd && (!e.termination_date || new Date(e.termination_date) > monthEnd)
      ).length;
    ...
    setAnalytics({ headcount, leaveRequests, totalSpent, genderDistribution, leaveByDept, attritionRate: attritionData, budgetVsActual });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: MATERIAL_COLORS.background }}>
        <div className="text-center">
          <div className="w-16 h-16 border-4 border-primary border-t-transparent rounded-full animate-spin mx-auto mb-4" style={{ borderColor: MATERIAL_COLORS.primary }}></div>
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <ChartCard
            ...
            subtitle="Workforce demographics overview"
          >
            <div className="h-80">
          ...
          <ChartCard
            title="Leave Requests"
            subtitle="By department for selected period"
          >
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={analytics.leaveByDept} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
                  ...
                  <Bar 
                    dataKey="requests" 
                    fill={MATERIAL_COLORS.secondary} 
                    radius={[4, 4, 0, 0]}
                  />
                </BarChart>
              </ResponsiveContainer>
          ...
          <ChartCard
            ...
            subtitle={`${format(filters.dateRange.from, 'MMM yyyy')} performance`}
          >
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className={`p-3 rounded-lg ${analytics.budgetVsActual.progress > 90 ? 'bg-red-50' : 'bg-green-50'}`}>
                  <p className={`text-sm font-medium ${analytics.budgetVsActual.progress > 90 ? 'text-red-700' : 'text-green-700'}`}>
                    {analytics.budgetVsActual.progress > 90 
        ...
        <ChartCard
          ...
          subtitle="Monthly turnover rate over the last 6 months"
        >
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
