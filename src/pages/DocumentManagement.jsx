...
import { HRDocument, HRFolder, User } from '@/api/entities';
...
export default function DocumentManagement() {
  ...
  const initializeDefaultStructure = useCallback(async () => {
    ...
    for (const folderName of defaultFolders) {
        const newFolder = await HRFolder.create({ name: folderName, parent_folder_id: null });
  ...
  const loadData = useCallback(async (isInitialLoad = false) => {
    ...
    try {
      ...
      if (isInitialLoad && foldersData.length === 0 && documentsData.length === 0) {
        ...
        const [newFolders, newDocs] = await Promise.all([HRFolder.list(), HRDocument.list()]);
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      await HRFolder.create({ name: newFolderName, parent_folder_id: currentFolder });
      setNewFolderName("");
  ...
  const handleFileUpload = async () => {
    ...
    setIsUploading(true);
    try {
      const { file_url } = await UploadFile({ file: uploadData.file });
  ...
  const handleAccessChange = async (doc, newLevel) => {
    try {
        const updatedDoc = await HRDocument.update(doc.id, { access_level: newLevel });
