import { Issue, Project, Sprint, User } from '@/api/entities';
...
export default function AgileDeskBoard() {
  ...
  const onDragEnd = async (result) => {
    ...
    const newIssues = issues.map((issue) => (issue.id === draggableId ? { ...issue, status: newStatus } : issue));
    setIssues(newIssues);
  ...
  const filteredIssues = issues.filter((issue) => filters.assignee === 'all' || issue.assignee_id === filters.assignee);

  const getIssuesByStatus = (status) => {
    return filteredIssues.filter((issue) => issue.status === status).sort((a, b) => (a.updated_date < b.updated_date ? 1 : -1));
  };
  ...
  if (!project) {
    return (
      <div className="p-8 text-center">
        Project not found. Go back to{' '}
        <Link to={createPageUrl('AgileDesk')} className="text-blue-600 hover:underline">
          AgileDesk
        </Link>
        .
      </div>
  ...
  return (
    <div className="p-4 lg:p-8 min-h-screen bg-gray-50 flex flex-col">
      ...
      {project.methodology === 'scrum' && activeSprint && <SprintHeader sprint={activeSprint} issues={issues} />}
                ...
                <Droppable key={status.id} droppableId={status.id}>
                  {(provided, snapshot) => (
                    <div className="w-80 flex-shrink-0">
                      <Card
                        className={`bg-gray-100/60 border-gray-200 ${snapshot.isDraggingOver ? 'shadow-lg bg-blue-50' : ''} transition-colors`}
                      >
                        ...
                        <CardContent ref={provided.innerRef} {...provided.droppableProps} className="p-3 space-y-3 min-h-[500px]">
                          {statusIssues.map((issue, index) => (
                            <Draggable key={issue.id} draggableId={issue.id} index={index}>
                              {(provided, snapshot) => (
                                ...
                                >
                                  <IssueCard issue={issue} teamMembers={teamMembers} onEdit={handleEditIssue} />
                                </div>
                              )}
                            </Draggable>
                          ))}
