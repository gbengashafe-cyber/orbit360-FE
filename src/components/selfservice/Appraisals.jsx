...
const AppraisalForm = ({ appraisal, appraisalKPIs, onUpdate, employee, currentUser, isSupervisorView }) => {
  ...
  const [overallComments, setOverallComments] = useState(
    isSupervisorView ? appraisal.supervisor_overall_comments : appraisal.employee_overall_comments,
  );
  ...
  const handleRatingChange = (id, rating) => {
    setKpiData(
      kpiData.map((kpi) =>
        kpi.id === id ? { ...kpi, [isSupervisorView ? 'supervisor_rating' : 'employee_rating']: parseInt(rating) } : kpi,
      ),
    );
  };
  ...
  const handleCommentChange = (id, comment) => {
    setKpiData(
      kpiData.map((kpi) =>
        kpi.id === id ? { ...kpi, [isSupervisorView ? 'supervisor_comment' : 'employee_comment']: comment } : kpi,
      ),
    );
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const kpiUpdatePromises = kpiData.map((kpi) =>
        appraisalService
          .updateAppraisalKPI(appraisal.id, kpi.id, kpi)
  ...
  const canEdit =
    (isSupervisorView && appraisal.status === 'pending_supervisor_review') ||
    (!isSupervisorView && appraisal.status === 'pending_self_assessment');
    ...
    <div className="space-y-6">
        ...
        <Card key={kpi.id}>
          <CardHeader>
            <CardTitle className="text-lg">
              {kpi.kpi_title} <span className="text-sm font-normal text-gray-500">- {kpi.kpi_weight}%</span>
            </CardTitle>
            ...
            <div>
              ...
              <div className="grid grid-cols-5 gap-4 mt-2">
                <div className="col-span-1">
                  ...
                  <Select
                    value={kpi.employee_rating}
                    onValueChange={(val) => handleRatingChange(kpi.id, val)}
                    disabled={isSupervisorView || !canEdit}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
      ...
      {canEdit && (
        <Button onClick={handleSubmit} disabled={isSubmitting}>
          ...
          {isSupervisorView ? 'Final Appraisal' : 'to Supervisor'}
        </Button>
      )}
    </div>
  );
...
export default function AppraisalsComponent({ employee, currentUser, onUpdate, isSupervisor, isHrAdmin, directReports }) {
    ...
    try {
      ...
      if (Array.isArray(kpiTemplates) && kpiTemplates.length > 0) {
        const kpiUpdatePromises = kpiTemplates.map((kpi) =>
          appraisalService
            .updateAppraisalKPI(newAppraisal.id, kpi.id, {
              kpi_id: kpi.id,
              kpi_title: kpi.title,
            ...
            <div key={appraisal.id} className="p-3 border rounded-lg flex justify-between items-center">
              <div>
                <p className="font-semibold">
                  {appraisal.cycle_name} {appraisal.employee_name && ` - ${appraisal.employee_name}`}
                </p>
                <Badge className={getStatusBadge(appraisal.status)}>{appraisal.status.replace(/_/g, ' ')}</Badge>
              </div>
              <Button variant="outline" onClick={() => setSelectedAppraisalId(appraisal.id)}>
                View
              </Button>
            </div>
          ))}
  ...
  if (selectedAppraisal && appraisalKPIs.length > 0) {
    return (
      <div>
        <Button variant="outline" onClick={() => setSelectedAppraisalId(null)} className="mb-4">
          ← Back to List
        </Button>
        <AppraisalForm
    ...
    <div className="space-y-6">
      {isSupervisor && (
        <div className="flex space-x-1 rounded-lg bg-gray-100 p-1">
          <Button
            onClick={() => setActiveTab('my_appraisal')}
            variant={activeTab === 'my_appraisal' ? 'default' : 'ghost'}
            className="w-full"
          >
            My Appraisals
          </Button>
          <Button
            onClick={() => setActiveTab('team_appraisals')}
            variant={activeTab === 'team_appraisals' ? 'default' : 'ghost'}
            className="w-full"
          >
            Team Appraisals
          </Button>
              ...
              <div className="flex items-center gap-4">
                <Select onValueChange={handleCreateAppraisal}>
                  <SelectTrigger className="w-[280px]">
                    <SelectValue placeholder="Select an active appraisal cycle..." />
                  </SelectTrigger>
                  <SelectContent>
